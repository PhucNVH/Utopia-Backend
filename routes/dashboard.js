                    ...
                    user.findById(reserve.username, function (err, User) {
                        if (err) console.log(err);
                            ...
                            roomDetail.find({ Reserve: reserve._id }, function (err, Room) {
                                ...
                                Data.push(billData);
                                length++;
                                if (allBill.length == length) { res.render("dashboard/billing.ejs", { Data: Data }); }
                            });
...
router.get("/dashboard/room", function (req, res) {
    res.render("dashboard/search.ejs", { Room: null, Data: null, From: null, To: null });
});
...
router.post("/dashboard/room", function (req, res) {
    if (req.body.type == null) res.redirect("/rooms");
        ...
        room.
            ...
            populate({ path: 'Detail', populate: { path: 'Reserve', match: { $or: [{ DateIn: { $gt: req.body.from, $lt: req.body.to } }, { DateOut: { $gt: req.body.from, $lt: req.body.to } }] } } }).
            exec(function (err, Room) {
                ...
                if (counter == 0) { console.log("No room left"); res.redirect("/rooms/" + req.body.Name, { info: { message: "No Room Left" } }); }
                else if (counter > 0) { res.render("dashboard/search.ejs", { Room: Data, Type: Type, From: req.body.from, To: req.body.to }); }
            });
    }
...
router.post("/dashboard/book", function (req, res) {
    ...
    roomDetail.findByIdAndDelete(req.body.RoomID, function (err, foundRoom) {
        if (err) { console.log(err); res.json({ success: false }) }
            ...
            ReserveInfo.create({ DateIn: req.body.from, DateOut: req.body.to, Children: req.body.children, Adult: req.body.adult, username: req.user },
